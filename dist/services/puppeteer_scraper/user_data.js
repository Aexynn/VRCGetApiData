...
async function saveData(page, url, dataDir, user_id) {
    ...
    try {
        ...
        fs_1.default.writeFileSync(path_1.default.join(dataDir, `${user_id}/user_data.json`), JSON.stringify(data.user, null, 2));
        fs_1.default.writeFileSync(path_1.default.join(dataDir, `${user_id}/worlds_data.json`), JSON.stringify(data.worlds, null, 2));
        fs_1.default.writeFileSync(path_1.default.join(dataDir, `${user_id}/groupsRepresented_data.json`), JSON.stringify(data.groups.represented, null, 2));
        ...
        if (await checkIfMidnight()) {
            ...
            fs_1.default.writeFileSync(path_1.default.join(dataDir, `${user_id}/groupsList_data.json`), JSON.stringify(data.groups.list, null, 2));
        }
        else {
...
(async () => {
    ...
    try {
        ...
        while (true) {
            ...
            const waitTime = Math.floor(Math.random() * (2 * 60 * 60 * 1000 - 1 * 60 * 60 * 1000)) +
                1 * 60 * 60 * 1000;
