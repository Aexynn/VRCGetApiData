...
async function saveData(page) {
  ...
  data.account.id = process.env.USER_ID;
  data.account.username = await page.$eval("h2.tw-text-h2", (el) =>
    el.textContent.trim()
  );
  data.account.user_rank = await page.$eval('div[title="User Rank"]', (el) =>
    el.textContent.trim()
  );
  data.account.last_login = await page.$eval(
    'div[title^="Last online"]',
    (el) => el.textContent.trim()
  );
  data.statusMsg = await page.$eval("p.tw-text-h4.tw-truncate.tw-m-0", (el) =>
    el.textContent.trim()
  );
  data.bio = await page.$eval("p.tw-touch-auto", (el) => el.textContent.trim());
...
(async () => {
  ...
  try {
    ...
    console.log("Change page to user page.");
